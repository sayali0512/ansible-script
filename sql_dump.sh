...
mysql -h $target_db_server_name -u $target_db_login_name -p$target_db_pass -e "use $target_db_name;SET SQL_SAFE_UPDATES=0;UPDATE wp_options SET option_value = replace(option_value, 'http://$source_dns_name', 'https://$target_dns_name') WHERE option_name = 'home' OR option_name = 'siteurl';UPDATE wp_posts SET guid = replace(guid, 'http://$source_dns_name','https://$target_dns_name');UPDATE wp_posts SET post_content = replace(post_content, 'http://$source_dns_name', 'https://$target_dns_name');"
